...
def get_info_for_movie(movie_name, use_cache=True):
    ...
    if invalid or not(use_cache):
        key = search(movie_title)
        if key is not None:
            data = get_info(key)
            if use_memcache: # this allows us to force refresh data if we want
                client.set(movie_title, json.dumps(dict(data=data, time_retrieved=int(time.time()))))
        else:
            data = None
    return data